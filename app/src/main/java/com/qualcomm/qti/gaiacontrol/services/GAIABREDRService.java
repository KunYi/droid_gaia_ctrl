...
public class GAIABREDRService extends Service implements BluetoothService, BondStateReceiver.BondStateListener {
    ...
    public boolean onUnbind(Intent intent) {
        ...
        if (mAppListeners.isEmpty() && !isUpgrading()) {
            disconnectDevice();
        }
    ...
    public void onCreate() {
        ...
        mGAIABREDRProvider.showDebugLogs(mShowDebugLogs);
    }
    ...
    public void enableDebugLogs(boolean enable) {
        ...
        mGAIABREDRProvider.enableDebugLogs(enable);
    }
    ...
    public void enableUpgrade(boolean enable) {
        mGAIABREDRProvider.enableUpgrade(enable);
    }
    ...
    public void onBondStateChange(BluetoothDevice device, int state) {
        ...
        if (device != null && connectedDevice != null && device.getAddress().equals(connectedDevice.getAddress())) {
            if (mShowDebugLogs) Log.i(TAG, "ACTION_BOND_STATE_CHANGED for " + device.getAddress()
    ...
    private void handleMessageFromProvider(Message msg) {
        ...
        switch (msg.what) {
            ...
            case GAIABREDRProvider.Messages.GAIA_PACKET:
                ...
                if (mShowDebugLogs) Log.i(TAG, handleMessage + "GAIA_PACKET");
                onGaiaDataReceived(data);
                break;

            case GAIABREDRProvider.Messages.GAIA_READY:
                if (mShowDebugLogs) Log.i(TAG, handleMessage + "GAIA_READY");
