...
public class MainGaiaManager extends AGaiaManager {
    ...
    /**
     * <p>To count how many of the commands to check had been tested to get the supported GAIA features.</p>
     */
    private int mCommandsChecked = 0;
    /**
     * <p>The number of commands to test to get the supported GAIA features.</p>
     */
     ...
     * is supported.</p>
     * <p>The answer to this method is asynchronous. The listener will get a call for the features which are
     * supported with {@link MainGaiaManagerListener#onFeatureSupported(int) onFeatureSupported(int)}. Once all
     * features had been tested {@link MainGaiaManagerListener#onFeaturesDiscovered() onFeaturesDiscovered()} is
     * called.</p>
     */
    public void getSupportedFeatures() {
        // to check if a command is supported we use the commands with no parameters
        ...
        // Features.EQUALIZER
        checkCommands(GAIA.COMMAND_GET_3D_ENHANCEMENT_CONTROL, GAIA.COMMAND_GET_BASS_BOOST_CONTROL,
                GAIA.COMMAND_GET_USER_EQ_CONTROL, GAIA.COMMAND_GET_EQ_CONTROL);
        ...
        // other commands are tested depending on the success of COMMAND_VM_UPGRADE_CONNECT
    ...
    protected void receiveUnsuccessfulAcknowledgement(GaiaPacket packet) {
        ...
        if (packet.getStatus() != GAIA.Status.NOT_SUPPORTED) {
            // Command is supported but its use is wrong
            onFeatureSupported(command);
            onInformationNotSupported(command); // we consider that the information is not supported here

            // in the case of the UPGRADE commands we try the other ones as well
            if (command == GAIA.COMMAND_VM_UPGRADE_CONNECT) {
                createRequest(createPacket(GAIA.COMMAND_VM_UPGRADE_CONTROL));
            }
            else if (command == GAIA.COMMAND_VM_UPGRADE_CONTROL) {
                createRequest(createPacket(GAIA.COMMAND_VM_UPGRADE_DISCONNECT));
            }
        }
        else if (command == GAIA.COMMAND_GET_LED_CONTROL || command == GAIA.COMMAND_GET_3D_ENHANCEMENT_CONTROL
                || command == GAIA.COMMAND_GET_BASS_BOOST_CONTROL || command == GAIA.COMMAND_GET_USER_EQ_CONTROL
                || command == GAIA.COMMAND_GET_EQ_CONTROL || command == GAIA.COMMAND_GET_TWS_AUDIO_ROUTING
                || command == GAIA.COMMAND_GET_TWS_VOLUME || command == GAIA.COMMAND_AV_REMOTE_CONTROL
                || command == GAIA.COMMAND_VM_UPGRADE_CONNECT || command == GAIA.COMMAND_VM_UPGRADE_CONTROL
                || command == GAIA.COMMAND_VM_UPGRADE_DISCONNECT) {
            // the command is not supported and has been tested
            onCommandChecked();
        }
        else {
    ...
    protected void hasNotReceivedAcknowledgementPacket(GaiaPacket packet) {
        int command = packet.getCommand();
        if (command == GAIA.COMMAND_GET_LED_CONTROL || command == GAIA.COMMAND_GET_3D_ENHANCEMENT_CONTROL
                || command == GAIA.COMMAND_GET_BASS_BOOST_CONTROL || command == GAIA.COMMAND_GET_USER_EQ_CONTROL
                || command == GAIA.COMMAND_GET_EQ_CONTROL || command == GAIA.COMMAND_GET_TWS_AUDIO_ROUTING
                || command == GAIA.COMMAND_GET_TWS_VOLUME || command == GAIA.COMMAND_AV_REMOTE_CONTROL
                || command == GAIA.COMMAND_VM_UPGRADE_CONNECT || command == GAIA.COMMAND_VM_UPGRADE_CONTROL
                || command == GAIA.COMMAND_VM_UPGRADE_DISCONNECT) {
            // the tested command did not received an acknowledgement
            onCommandChecked();
        }
    }
     ...
     * {@link com.qualcomm.qti.libraries.gaia.GAIA.NotificationEvents#CHARGER_CONNECTION CHARGER_CONNECTION}.</p>
     ...
     *          {@link GAIA#COMMAND_EVENT_NOTIFICATION COMMAND_EVENT_NOTIFICATION} and the event
     *          {@link com.qualcomm.qti.libraries.gaia.GAIA.NotificationEvents#CHARGER_CONNECTION CHARGER_CONNECTION}.
     *
     ...
     * event {@link com.qualcomm.qti.libraries.gaia.GAIA.NotificationEvents#CHARGER_CONNECTION CHARGER_CONNECTION}.</p>
    ...
    /**
     * <p>This method create requests with the given commands in order to test if they are supported. It increments
     * the counter of commands to check.</p>
     *
     * @param commands
     *          The list of commands to test.
     */
    private void checkCommands(int... commands) {
        COMMANDS_TO_CHECK += commands.length;
        for (int command : commands) {
            createRequest(createPacket(command));
        }
    }

    /**
     * T<p>This method is called to validate that a command had been tested. This method increments the counter of
     * tested commands and calls {@link MainGaiaManagerListener#onFeaturesDiscovered() onFeaturesDiscovered()} if
     * they all have been tested.</p>
     */
    private void onCommandChecked() {
        mCommandsChecked++;
        if (mCommandsChecked == COMMANDS_TO_CHECK) {
            mListener.onFeaturesDiscovered();
            COMMANDS_TO_CHECK = 0;
        }
    }
    ...
    public interface MainGaiaManagerListener {
         ...
         * through the
         * {@link com.qualcomm.qti.libraries.gaia.GAIA.NotificationEvents#CHARGER_CONNECTION CHARGER_CONNECTION} event.</p>
         *
         * @param isConnected
        ...
        /**
         * <p>To inform the listeners that all features have been tested after it has called
