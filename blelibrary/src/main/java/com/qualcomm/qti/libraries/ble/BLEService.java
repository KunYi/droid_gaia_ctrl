...
public abstract class BLEService extends Service {
    ...
    /**
     * <p>The maximum size the MTU can be.</p>
     */
    protected static final int MTU_SIZE_MAXIMUM = 512;
    /**
     * <p>The default size of the MTU.</p>
     */
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            receiveMtuChanged(gatt, mtu, status);
        }
    };
    ...
    /**
     * <p>To request the MTU size to be negotiated with the remote size starting with the given size if this service
     * is connected to a remote device.</p>
     * <p>If this method returns true, the subclass will get the response to this request asynchronously through
     * the method {@link #onMTUChanged(BluetoothGatt, int, int) onMTUChanged}.</p>
     *
     * @param mtu
     *          The size to start the negotiations with.
     *
     * @return <p>false if it is not possible to request the given MTu size. The reasons could be:
     * <ul>
     *     <li>The device is not connected./li>
     *     <li>The given MTU size is not between 23 and 512 which are the minimum and maximum possible values.</li>
     *     <li>The Android version is not compatible, minimum is Lollipop.</li>
     * </ul>
     * Returns true if the request could be added to the requests queue.</p>
     */
    @SuppressLint("ObsoleteSdkInt")
    protected boolean requestMTUSize(int mtu) {
        if (mShowDebugLogs) {
            Log.d(TAG, "Request received for request MTU.");
        }

        if (mConnectionState != State.CONNECTED) {
            Log.w(TAG, "request MTU not initiated: device is disconnected.");
            return false;
        }

        if (mDevice == null) {
            Log.w(TAG, "request MTU not initiated: device is null.");
            return false;
        }

        if (mtu < MTU_SIZE_DEFAULT || mtu > MTU_SIZE_MAXIMUM) {
            Log.w(TAG, "request MTU not initiated: value (" + mtu + ") not in interval [" + MTU_SIZE_DEFAULT + ", " +
                    MTU_SIZE_MAXIMUM + "].");
            return false;
        }

        if (android.os.Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            Log.w(TAG, "request MTU not initiated: Android version is too low, minimum required is Lollipop.");
            return false;
        }

        // all check passed successfully, the request can be initiated
        Request request = Request.createMTURequest(mtu);
        addToRequestsQueue(request);
    ...
    /**
     * <p>This method is called when the asynchronous answer to the {@link #requestMTU(int) requestMTU}
     * method had been received.</p>
     *
     * @param gatt
     *              The Bluetooth gatt which requested the remote rssi.
    ...
    /**
     * <p>This method is called when the Bluetooth GATT client receives a value for a MTU request.</p>
     *
     * @param gatt
     *              The Bluetooth gatt which received the information.
     * @param mtu
     *              The mtu value which had been chosen.
     * @param status
     *              The status of this characteristic reading: {@link BluetoothGatt#GATT_SUCCESS} if the operation
     *              succeeds.
     */
    private void receiveMtuChanged(BluetoothGatt gatt, int mtu, int status) {
        if (mShowDebugLogs) {
            Log.d(TAG, "GattCallback - onMTUChanged, mtu=" + mtu + " status=" + status);
        }

        Request request = onReceiveCallback(Request.RequestType.REQUEST_MTU);
        boolean expectedCallback = request != null;

        if (status != BluetoothGatt.GATT_SUCCESS) {
            Log.w(TAG, "Unsuccessful MTU request - status: " + BLEUtils.getGattStatusName(status, false));
            if (expectedCallback) onRequestFailed(request);
        }
        else if (expectedCallback) {
            processNextRequest();
    ...
    /**
     * <p>Request the MTU size to be negotiated with the remote device starting with the given MTU size.
     * The read result is reported asynchronously through
     * the {@code BluetoothGattCallback{@link #onMTUChanged(BluetoothGatt, int, int) onMTUChanged} callback.</p>
     *
     * @param mtu
     *          The size to start the negotiations with.
     *
    ...
    private boolean requestMTU(int mtu) {
        if (mShowDebugLogs) {
            Log.d(TAG, "Process request MTU");
        }
        if (mBluetoothAdapter == null) {
            Log.w(TAG, "Request MTU cannot be processed: BluetoothAdapter is null.");
            return false;
        }
        if (mBluetoothGatt == null) {
            Log.w(TAG, "Request MTU cannot be processed: BluetoothGatt is null.");
            return false;
        }

        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            boolean done = mBluetoothGatt.requestMtu(mtu);
            if (mShowDebugLogs) {
                Log.d(TAG, "Request read remote RSSI dispatched to system: " + done);
            }
            return done;
            ...
            Log.w(TAG, "Request MTU cannot be processed: requires at least Android Lollipop API 21.");
            return false;
        }
    }
