...
public class UpgradeManager {
     ...
     */
    private int mMaxLengthForDataTransfer = OpCodes.UpgradeDataBytesREQ.DATA_LENGTH;
    /**
     ...
     * {@link com.qualcomm.qti.libraries.vmupgrade.codes.OpCodes.Enum#UPGRADE_DATA_BYTES_REQ UPGRADE_DATA_BYTES_REQ} request.
     */
    ...
    /**
     * <p>To know if this manager must restart an upgrade when the upgrade had been disconnected.</p>
     */
    private boolean hasToRestartUpgrade = false;
    /**
     * <p>To know if this manager can throw more than one packet at a time during the data transfer or if it should
     * wait for an acknowledgement from the listener first - through
     * {@link #onSuccessfulTransmission() OnSuccessfulTransmission}.</p>
     */
     ...
     * dispatch any received Upgrade messages.</p>
     *
     * @param listener
    ...
    public UpgradeManager(@NonNull UpgradeManagerListener listener) {
     ...
     *
     * @deprecated As of 1.1.0 it is recommended to use {@link #UpgradeManager(UpgradeManagerListener)
     * UpgradeManager(UpgradeManagerListener)} and to set up the maximum length when starting the upgrade with
     * {@link #startUpgrade(int, boolean) startUpgrade(int, boolean)}.
     */
    @SuppressWarnings({"SameParameterValue", "WeakerAccess"})
    // maxLength is always set for BLE packets for this application
    public UpgradeManager(@NonNull UpgradeManagerListener listener, int maxLength) {
        this.mListener = listener;
        this.mMaxLengthForDataTransfer =  maxLength - VMUPacket.REQUIRED_INFORMATION_LENGTH;
    }
    ...
    /**
     * <p>To set up the file to use for the upgrade process.</p>
     *
     * @param file
     *          The binary file to transfer onto the board to update it.
     */
     ...
     * </ul></p>
     *
     * @deprecated As of release 1.1.0 it is replaced by {@link #startUpgrade(int, boolean) startUpgrade(int, boolean)}.
     */
    @SuppressWarnings("WeakerAccess")
    public void startUpgrade() {
        Log.w(TAG, "startUpgrade() is deprecated, please use startUpgrade(int maxLength, boolean sendMultiplePackets)" +
                " instead.");
    }
     ...
     *
     * @param maxLength
     *      The maximum length the VMU packets can have - depends in general on the communication protocols.
     *      <i>For example using BLE and GAIA, the maximum could be 16.</i>
     *
     * @param sendMultiplePackets
     *      True if this manager can throw more than one packet at a time during the data transfer or needs to wait
     *      for a first packet to be transmitted successfully before to send a second one, listener must use
     *      {@link #onSuccessfulTransmission() onSuccessfulTransmission}.
     *
     * @since 1.1.0
     */
    @SuppressWarnings("WeakerAccess")
    public void startUpgrade(int maxLength, boolean sendMultiplePackets) {
        // set up parameters
        if (maxLength < OpCodes.UpgradeDataBytesREQ.DATA_LENGTH) {
            Log.w(TAG, "setPacketMaxLengthForDataTransfer: given length is too short, minimum value is setup: "
                    + OpCodes.UpgradeDataBytesREQ.DATA_LENGTH);
            this.mMaxLengthForDataTransfer = OpCodes.UpgradeDataBytesREQ.DATA_LENGTH;
        }
        else {
            this.mMaxLengthForDataTransfer = maxLength - VMUPacket.REQUIRED_INFORMATION_LENGTH;
        }

        mSendMultiplePackets = sendMultiplePackets;
     ...
     * @deprecated from 1.1.0, replaced by {@link #onSuccessfulTransmission() onSuccessfulTransmission()};
     */
    public void receiveVMControlSucceed() {
        Log.w(TAG, "method receiveVMControlSucceed is deprecated, please use onSuccessfulTransmission() instead.");
    }

    /**
     * This method is called when we received a successful transmission from the board for VMU packets
     * which have been sent. This method is required for the upgrade process because some of the VMU packets don't not
     * have an answer from the board. To continue the process the manager has to know they have been successfully
     ...
     * upgrade process.
     *
     * @deprecated from 1.1.0, replaced by {@link #onUpgradeDisconnected() onUpgradeDisconnected()}.
     */
    public void receiveVMDisconnectSucceed() {
        Log.w(TAG, "method receiveVMDisconnectSucceed() is deprecated, please use onUpgradeDisconnected() instead.");
    }
    ...
    /**
     * <p>This method starts the upgrade process. It checks that there isn't already a upgrade going on. It resets
     * the manager to start the upgrade and sends the
     * {@link com.qualcomm.qti.libraries.vmupgrade.codes.OpCodes.Enum#UPGRADE_SYNC_REQ UPGRADE_SYNC_REQ} to start the
     * process.</p>
     * <p>This method can dispatch a VMUError object if the manager has not been able to start the upgrade process.</p>
     * <p>The possible {@link UpgradeError.ErrorTypes ErrorTypes} are the following:
     * <ul>
    ...
    private void startUpgradeProcess() {
        if (!isUpgrading && mBytesFile != null) {
            isUpgrading = true;
            resetUpload();
            sendSyncReq();
        }
     ...
     *          The packet to send.
     */
    private void sendVMUPacket(VMUPacket packet, boolean isTransferringData) {
        byte[] bytes = packet.getBytes();
        if (isUpgrading) {
            if (mShowDebugLogs)
                Log.d(TAG, "send " + OpCodes.getString(packet.getOpCode()) + ": " +
                        VMUUtils.getHexadecimalStringFromBytes(bytes));
            mListener.sendUpgradePacket(bytes, isTransferringData);
        }
        else {
    ...
    /**
     * To calculate the elapsed time and percentage of upload done, and inform the listener about them.
     */
    private void onFileUploadProgress () {
        double percentage = mStartOffset * 100.0 / mBytesFile.length;
        percentage = (percentage < 0) ? 0 : (percentage > 100) ? 100 : percentage;
        mListener.onFileUploadProgress(percentage);
    }

    /**
     * To send the next data packet depending on the number of bytes requested by the Device through its last
     * {@link com.qualcomm.qti.libraries.vmupgrade.codes.OpCodes.Enum#UPGRADE_DATA_BYTES_REQ} request.
     */
    ...
    private void receiveSyncCFM(VMUPacket packet) {
        ...
        if (data.length >= OpCodes.UpgradeSyncCFM.DATA_LENGTH) {
            ...
            if (step == ResumePoints.Enum.IN_PROGRESS) {
                setResumePoint(step);
            }
            else {
                mResumePoint = step;
            ...
            mResumePoint = ResumePoints.Enum.DATA_TRANSFER;
        }
        sendStartReq();
    }
    ...
    public interface UpgradeManagerListener {
         ...
         */
        void sendUpgradePacket(byte[] bytes, boolean isTransferringData);
         ...
         * the elapsed time and the percentage accomplished.</p>
         *
         * @param percentage
         *          The percentage of how many bytes from the file have been sent to the Board.
         */
        void onFileUploadProgress(double percentage);
